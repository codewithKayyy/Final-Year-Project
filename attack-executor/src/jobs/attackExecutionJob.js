...
const attackWorker = new Worker(ATTACK_QUEUE_NAME, async (job) => {
    ...
    try {
        ...
        // TODO: Send results back to main backend or update database directly
        // Example: Make an HTTP POST request to main backend /api/simulations/update-status
        // axios.post("http://localhost:5000/api/simulations/update-attack-log", {
        //     simulationId, agentId, scriptId, status: "completed", stdout, stderr
        // });

        return { status: "success", stdout, stderr };
    } catch (error) {
        console.error(`Job ${job.id} failed for script ${scriptId}:`, error.message);
        // TODO: Send error status back to main backend or update database
        // axios.post("http://localhost:5000/api/simulations/update-attack-log", {
        //     simulationId, agentId, scriptId, status: "failed", error: error.message
        // });
